...
- **Hold-outs were built before the arms ran**, and deliberately not tuned against
  afterwards, which would convert the hold-out into training data.
...
**Result:** `gold_coverage_avg` 0.675 → 0.825, moving the full distance to its ceiling, so
...
**Question:** one row's gold page (`dpr.pdf` p3) sat outside the whole question's top-50, so
...
both fell, because the halves order the top of the slate worse, and the answers got better
anyway, because `RERANK_K=3` picks from a 12-page slate, so what gates the answer is whether
gold is in the slate at all. Adopting this lowered two gate floors; that is the real price.
...
**Result: eight of nine alternatives lost outright.** The ninth beat the shipped reduction on
raw AUC, by an amount that is not significant and does not survive the smoothing below.
...
per-patch z-scoring is far below chance, inverting the signal. The most principled candidate,
...
- **`RENDER_DPI` cannot speed up embedding.** A 150-DPI page is 2.1 MP, and `smart_resize` hands the
  model 672×868, so it already reads pages at ~79 DPI equivalent. DPI buys render time and disk only.
...
chip, because trace placement is what AUC 0.629 supports.
...
1.0, because a question one page can answer still scores perfectly whenever retrieval happens to
reach both documents, so `gold_doc_coverage` structurally cannot surface it. Only reading the
corpus can.
...
legacy absolute paths, so no re-ingest and no `EMBED_VERSION` bump, since this is payload rather
than vector data); `_sync` treating missing page images as stale, which makes a plain re-ingest the
repair; and `index_health()` reporting the split at boot and on `/health`.
...
`translateX(-100%)`, because a translated-away element is still in the tab order, so the first
Tab past the hamburger vanishes into a drawer nobody can see. And the drawer is a disclosure,
...
renders two per document across the 19-document corpus. Landmarks do not help there, since they
are a screen-reader affordance and that is a sighted keyboard-only path, so there is a skip link.
...
- **Three metrics are back on the ceiling** (`recall@12`, `rerank_recall`, `abstention_accuracy`)
  with no observed headroom in the current baseline. Their configured gates remain active and can
  fail after a regression, but three of the ten are currently un-trippable. Re-de-saturating
...
- **The shipped confidence statistic is the weakest one that works; the swap is scoped, unspent.**
  Tested offline against `calib_baseline.json` (73 rows, 24 negatives, label `gold_rank == 1`):
  `zscore_top1` AUC 0.8078, `margin_mean` 0.7908, `margin_rel`/`ratio_top2` 0.7874, the shipped
  `softmax_top1` 0.6293, and score entropy 0.4949, which is no signal at all. All p < 0.0001 except
...
- **`/images` is unauthenticated with guessable paths** (`<stem>_page_<n>.png`). Deliberate, since
  an `<img src>` cannot send a header, and fine for a demo corpus, but a confidentiality leak the
