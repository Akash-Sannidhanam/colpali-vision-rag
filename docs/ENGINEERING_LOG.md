...
- **`Dockerfile`** *(new)* + **`.dockerignore`** *(new)*: multi-stage `uv` build
  ...
  **auto-falls back to CPU**. `COPY --chown` (not a trailing `chown -R /app`) keeps the
  image at 11 GB instead of 21.7 GB, because the recursive chown would re-copy the
...
- **Incremental sync is the default** (`ingest.run_ingest`, `vector_store.live_collection`):
  it upserts into the live collection and embeds only documents whose fingerprint moved.
...
the eval into a sensitive regression instrument. The eval was saturated, with every
...
(recall@10 = 1.0, rerank_recall = 1.0, citation_accuracy = 1.0), so the reranker recovers
...
14786 ms. Retrieval metrics are byte-identical, as they must be, since `RERANK_K` is applied
...
- **`candidate_coverage_avg` is 0.975 at k=50, against 0.700 at k=10.** Retrieval reaches
  ...
  ceiling is therefore not ColQwen2's ranking ability; the second document is ranked, and
  crowded out of a 10-slot slate. That rules out "the model cannot find it" and
...
- `xdoc-rag-generator-dpr-dim` lost its citation by declining to cite at all rather than
  by citing wrongly, which is arguably better behaviour, scored as a regression.
...
since re-running an arm reproduced it digit for digit):
...
both fell, because the halves order the top of the slate worse than the whole question did,
and every answer-level metric improved anyway. `RERANK_K=3` picks from a 12-page slate, so
...
`candidate_coverage_avg` rose to 0.850, so the slate pass's headline property, that rerank
...
Adopting this lowered two floors, and that is the real price. `recall@1` and `recall@3`
...
- **`RENDER_DPI` cannot speed up embedding.** A 150-DPI page is 1275×1650 = 2.1 MP and
  `smart_resize` hands the model 672×868, so it already reads pages at ~79 DPI equivalent. DPI
...
- **The `×uniform` framing assumes the softmax share is the right statistic at all.** With
  ...
  are testable offline against `calib_baseline.json` for free, and that is the cheap next
  experiment, since it now needs no live arm to run.
...
| `image_path` stored relative to `PAGE_IMAGES_DIR` | the corpus relocates; reads still accept legacy absolute paths, so no re-ingest and no `EMBED_VERSION` bump, since this is payload rather than vector data |
...
**Measured against a live server**, over three interleaved rounds. The first attempt
...
- **`asyncio.Lock` wakes waiters FIFO.** `_yield_slice`'s re-acquire therefore lands behind
  exactly the requests queued at that instant and ahead of any arriving later, so ingest
...
read (honest, since `checked: 0` says nothing was verified, so `ok` claims only "no evidence of
...
The obvious repair, adding the alternate source to the row's gold, is backwards.
...
but 14 calibration keys. That is not a rescore bug: the report predates the
...
The pipeline never reads a text layer, which is the premise, but labeling may, exactly
...
it was reading. Pass `-i` to suppress them. Nothing was lost, since the 16 pinned distractors still
...
candidate and the one that most deserved to win, scored 0.5222.
